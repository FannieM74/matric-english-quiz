...
const TEXT_DIR = "/tmp/papers/text";
...
function normalizeAnswer(text) {
  if (!text) return "";
  return text.replace(/[A-Z]\)\s*/g, "").replace(/^\s+|\s+$/g, "");
}
...
async function parsePaper1_2025(text) {
  const qs = [];
  let qId = 154;
...
The wide range of funding sources accessed by participants to start their businesses underscores individual and collective agency, showing that institutions and networks play a pivotal role in nurturing youth entrepreneurship. By offering seed-funding, grants or loans, public and private stakeholders can help entrepreneurs launch and scale their ventures. Training programmes at institutions could cover essential skills such as business management, market identification, strategic planning and effective networking to prepare entrepreneurs for the challenges of the business world. Governments could look into creating enabling environments for upcoming entrepreneurs, ensuring that registration requirements are simple and accessible through online platforms.

The stories of successful entrepreneurs like Thandi offer valuable insights and lessons for young people who are contemplating entrepreneurship. In developing countries, the pressing need to diversify income sources further propels entrepreneurial ambitions. This sentiment was echoed by a 31-year-old male participant from Uganda, who remarked, "So, from our upbringing, we are often told you cannot just rely on your salary. From the start, we are taught that you have to do something on the side."
...
Upload a 30-40 second promo video showcasing your business.
Tag @MTNZA and use #MTNPulse #YouthHustleHub #LetsOperate for a chance to win.
Entries close 13 September. T&Cs apply.`),
  ...
  const p3 = await prisma.passage.create({
    data: {
      ...
      text: cleanText(`Sporting events have a unique ability to evoke intense emotions and excitement in fans worldwide. Sports can generate a spectrum of emotions that are unparalleled in other forms of entertainment. But what is it about sporting events that make them so exhilarating?
...
One of the main reasons people get so excited about sporting events is the deep sense of loyalty they feel towards their teams. Fans who strongly identify with their team may feel more satisfied and have a greater sense of belonging, which boosts their excitement and emotional investment in games. This sense of belonging can also make fans more resistant to negative information and more likely to talk positively about their team, strengthening the communal and emotional bonds that make sports so thrilling.
...
Additionally, broadcasters often use slow-motion replays, freeze frames and selective highlighting of intense moments to transform violent clashes into seemingly artistic performances. This not only captivates viewers but also frames the violence within a narrative of heroism and competition, enhancing the dramatic appeal. The narrative structure of sports events often mirrors classical drama, with a clear beginning, middle and end, and includes rising action, climax and resolution. This structure helps fans follow and become emotionally invested in the unfolding drama, seeing themselves as part of the story.
...
The excitement fans experience during sporting events is a complex interplay of multiple factors.`),
  ...
  qs.push(
    { id: qId++, passageId: p3.id, question: "Summarise, in your own words, the factors that inspire people to become sport fanatics. Include SEVEN points, NOT exceeding 90 words. Write a fluent paragraph. Indicate your word count at the end.", questionType: "essay", topic: "paper-1", section: "summary", year: 2025, examSession: "November 2025 NSC", marks: 10, explanation: "Summary must: be in point form using full sentences, include exactly 7 points, not exceed 90 words, be written as a fluent paragraph, and have word count indicated at the end." },
  );
...
Composting — everyone's doing it. Food scrap collection is now available citywide. Look for the green carts.`),
  ...
  const p5 = await prisma.passage.create({
    data: {
      ...
      text: cleanText(`ZITS cartoon by Jerry Scott and Jim Borgman. FRAME 1: Jeremy (thinking): "What do you mean you can't afford to give me money for a birthday present? I HAVE been saving my allowance for months!" FRAME 2: Father (calm, reassuring): "It's not the gift. It's the context that counts." FRAME 3: Jeremy (thinking): "With my own money I earned working at my first real summer job." FRAME 4: Mother (supportive): "Don't worry, we'll figure this out." FRAME 5: Jeremy opens a birthday card — inside is money. FRAME 6: Jeremy (relieved): "Maybe they can afford it after all." Glossary: 'Fishing lure' — a harmless device, often made of plastic or metal, used to attract fish (not the hook).`),
      order: 5,
    }
  });
...
async function parsePaper2_2025(text) {
  let qId = 200;
...
mama`),
  ...
  const childShotQs = [
    ...
    { id: qId++, passageId: p3.id, question: "Refer to lines 11-13: 'lying in the street / small and already a fist / with clenched eyes'. Evaluate the effectiveness of the image of the dead child.", questionType: "short", correctAnswer: null, marks: 4, topic: "paper-2", section: "poetry-child-shot", year: 2025, examSession: "November 2025 NSC", explanation: "The image is powerful because it juxtaposes vulnerability ('small', 'clenched eyes') with resistance ('a fist'), suggesting the child's silent protest even in death." },
  ];

  for (const q of childShotQs) {
    await prisma.question.create({ data: q }).catch(() => {});
  }
...
Must give us pause.`),
  ...
  console.log(`Paper 2 (2025): 5 passages, ${unseenQs.length + sonnetQs.length + childShotQs.length + novelQs.length + hamletQs.length} questions seeded`);
}

async function parsePaper3_2025(text) {
  let qId = 300;
...
async function seedAll() {
  ...
  await prisma.passage.deleteMany({}).catch(() => {});
